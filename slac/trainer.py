...
class Trainer:
    ...
    def debug_save_obs(self, state, name, step=0):
        self.writer.add_image(
            f"observation_{name}", state.astype(np.uint8), global_step=step
        )
    ...
    def train(self):
        ...
        self.env.unwrapped.sim.render_contexts[0].vopt.geomgroup[
            :
        ] = 1  # render all objects, including hazards
        ...
        for step in bar:
            t = self.algo.step(
                ...
                self.ob,
                t,
                (not self.collect_with_policy)
                and step <= self.initial_collection_steps,
                self.writer,
            )
        ...
        bar = tqdm(
            range(
                self.initial_collection_steps + 1,
        ...
        for step in bar:
            ...
            if step_env % self.algo.epoch_len == 0:
                self.writer.add_scalar(
                    "cost/train",
                    np.mean(self.algo.epoch_costreturns),
                    global_step=step_env,
                )
                self.writer.add_scalar(
                    "return/train",
                    np.mean(self.algo.epoch_rewardreturns),
                    global_step=step_env,
                )
    ...
    def evaluate(self, step_env):
        ...
        for i in range(self.num_eval_episodes):
            self.algo.z1 = None
            self.algo.z2 = None
            self.env_test.unwrapped.sim.render_contexts[0].vopt.geomgroup[
                :
            ] = 1  # render all objects, including hazards
            ...
            while not done:
                ...
                if i == 0 and eval_step % video_spf == 0:
                    ...
                    track = self.env_test.unwrapped.sim.render(
                        **render_kwargs
                    )[::-1, :, :]
                ...
                self.env_test.env.env.sim.render_contexts[0].vopt.geomgroup[
                    :
                ] = 1  # render all objects, including hazards
        ...
        self.writer.add_scalar(
            "return/test_median", median_reward_return, step_env
        )
        self.writer.add_scalar("cost/test", mean_cost_return, step_env)
        self.writer.add_scalar(
            "cost/test_median", median_cost_return, step_env
        )
        self.writer.add_histogram(
            "return/test_hist", np.array(reward_returns), step_env
        )
        self.writer.add_histogram(
            "cost/test_hist", np.array(cost_returns), step_env
        )

        print(
            f"Steps: {step_env:<6}   "
            f"Return: {mean_reward_return:<5.1f} "
            f"CostRet: {mean_cost_return:<5.1f}   "
            f"Time: {self.time}"
        )
